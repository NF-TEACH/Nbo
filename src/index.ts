...
import { MessageForwarder } from './forwarding/messageForwarder';
import { SummaryReporter } from './messages/summaryReporter';
import { describeMessage } from './messages/messageDescriptor';

const AUTH_FOLDER = 'auth';

const shouldHandleMessage = (message: WAMessage, forwarder: MessageForwarder): boolean => {
  if (!message.message) {
    return false;
  }
...
const start = async (): Promise<void> => {
  ...
  socket.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      logger.warn({ shouldReconnect, statusCode: error?.output?.statusCode }, 'Connection closed');
